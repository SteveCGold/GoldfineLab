...
%  See also:
%  FISHERDISC_TEST, FISHERDISC, GNORMCOR.
%
...
for is=1:2
    ...
    for ipart=1:size(fpart,1)
        ...
        if length(fbins)>0
            ifeat=ifeat+1;
            for ig=1:2
                  scenarios{is}.data{ig}(ifeat,:)=mean(spectra{is,ig}(fbins,:),1); %AG- this line imports the data
%                 scenarios{is}.data{ig}(ifeat,:)=mean(spectra{is,ig}(fbins,[2:2:62]),1); %this looks at every other point.
            end
        end %length(fbins)
    end %ipart
    for ig=1:2
...
for isp=1:length(islist)
    is=islist(isp);
    sc=scenarios{is};
    tags=[repmat(1,[1 sc.nsamps(1)]),repmat(2,[1 sc.nsamps(2)])]; %AG I need to create these tags to tell it which data is from which sample. consider
    %switching 1 and 2 so that it's condition 1 subtracting condition 2
    %(currently it's designed so condition 1 is the basline and looking at
    %how condition 2 compares to it)
    %
    % do the Fisher analysis
    %
    [results{isp},ou{isp}]=fisherdisc([sc.data{1},sc.data{2}],tags,opts); %AG- this is the line of code i need. [] puts data
    %arrays together. sc.data{1} is first condition. features are by row
    %and columns are trials.
    %
    % plot discriminants
    %
    scrsz = get(0,'ScreenSize'); %AG - for defining the figure size, need to change if want on a 2nd monitor
    figure;
%     set(gcf,'Position',[100 150 1300 900]);
    set(gcf,'Position',[1 1 scrsz(3)*0.8 scrsz(4)*0.9]); %AG
    set(gcf,'NumberTitle','off');
    set(gcf,'Name',sc.name);
    subplot(2,1,1);
    for isigsize=1:1 %for compatibility with fisherdisc_test (no "sigsize" here)
        rj=results{isp};
        hp(1)=plot([1:sc.nfeat],sc.signal/sqrt(sum(sc.signal.^2)),'k-','LineWidth',1);hold on; %AG - why is this done to the signal before plotting? to normalize it?
        ...
        if opts.xvalid==1
            ...
            labels{3}='debiased'; %AG - what is this thick debiased line?
