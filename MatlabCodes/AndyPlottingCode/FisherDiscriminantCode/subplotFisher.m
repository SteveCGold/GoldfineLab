function subplotFisher(figureTitle,filename1, pathname1,filename2, pathname2)
...
if ~exist('opts') %not sure that I should have this as an if or just reset each time ?
    ...
    opts.nshuffle_max=200; %could try 10000, doesn't matter. What atters is nflips;
    opts.sub1=1;
...
if nargin < 2
    [filename1, pathname1] = uigetfile('*PS.mat', 'Select first condition');
%     condition{1}=load(fullfile(pathname1,filename1));
    
    [filename2, pathname2] = uigetfile('*PS.mat', 'Select second condition');
...
if exist('fpart.mat','file')==2 %if fpart.mat is in the folder
    ...
    useDefault=[];
    disp('using Default fpart');
    disp(defaultfpart);
%     useDefault=input(['Use default frequency bins? (' defaultfpart ') -
%     (Return for Yes or n): '], 's'); %take out 9/1/10 for faster batching
...
%         if savefpartInWorkspace=='y'
            save fpart fpart %want it saved to folder for use next time %6/27 save as default
%         end
    end
end
...
if ~isfield(condition{1},'params')
    mt_params.tapers=[3 5];
    mt_params.Fs=256;
    mt_params.err=[2 0.05];
    mt_params.fpass=[0 100];
    mt_params.pad=-1;
    disp('params not present in PS file, using default values');
    disp(mt_params);
else
...
%         xlabel('feature','FontSize',12);
        ...
        set(gca,'YLim',[-1 1]);
        if j==numChannels %so only put one legend
...
%         title(cat(2,sc.name,sprintf(': segsize=%3.0f delrad=%3.0f',segsize,delrad)),'FontSize',10);
    ...
    figureTitle(figureTitle==' ')=[]; %to remove spaces
    savename=[figureTitle '_FisherData'];
%     savename=['Fisherdisc_' scenario_name]; 
     if exist([pathname1 savename '.mat'],'file') ==2; %check to see if file exists of that savename
...
saveas(figureHandle,[fullfile(pathname1,figureTitle) '_Fisher'],'fig');

save (fullfile(pathname1,savename),'results','ou','p_segflip','percentCorrectMapbayesSegs','percentCorrectMapbayesUnrestricted','channellabels','summarytable','fpart','filename1', 'pathname1','filename2', 'pathname2');
end