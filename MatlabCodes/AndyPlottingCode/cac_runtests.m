function cac_runtests(sched,level)
%cac_runTests(sched,level) - test function to test cacscheduler Install.
...
%  cd CACSCHEDULERHOME;
%  cacsched;
%  cac_runtests(sched,3);

%We know runTests must be on the path, so we use that.
CACHome = fileparts(which('cac_runtests'));
...
if level == 3 || level ==0
    fprintf('Running integration tests that simulate actual use of Red Cloud by a user.\n');
    getFreeCores();
...
%End function
    ...
    function runUserHomeTest(sched)
        ...
        if ~strcmp(ud1{3},ud2{3})
            ...
            userHome = ud1{3};
            n = regexp(userHome,'\\matlabstorage01.cac.cornell.edu\\matlab\\(?<userName>\w+)','names');
    ...
    function out = runJavaClassPathTest(sched)
        ...
        CACHome = fileparts(which('cac_runTests'));
        %Ensure we have a cacscheduler.jar and cxf-*.jar in there somewhere and
        %that they exist.
        cxfOK = 0;
        ljOK = 0;
        for i = 1:length(jars)
            if ~isempty(strfind(jars{i},'cxf-'))
                %See if the path is real, this is easiest done with a java File.
                jf =  java.io.File(jars{i});
                if ~jf.exists()
                    fprintf('MATLAB has not found a cxf jar file\n');
                else
            ...
            if ~isempty(strfind(jars{i},'cacscheduler.jar'))
                jf = java.io.File(jars{i});
                if ~jf.exists()
                    jf_real = java.io.File(fullfile(CACHome,'lib','cacscheduler.jar'));
                    warning('Found cacscheduler.jar in the javaclasspath, but it doesn''t seem to exist on your file system.\n');
                    fprintf('MATLAB has loaded %s, which says cacscheduler.jar is located %s but I can''t find it there.\n', cp);
                    if jf_real.exists()
                        fprintf('I found cacscheduler.jar here:%s, please change classpath.txt to reflect that path.',char(jf_real));
                    end
                else
                    %fprintf('Found %s, classpath looks OK\n', char(jf.getAbsolutePath()));
                    ljOK = 1;
    ...
    function rungridFTPTest(sched)
        ...
        ftp = gridFTP();
        ftp.list('',1);
        localPath = fullfile(CACHome,'cac_runtests.m');
        %Check to see if the remote file already exists
        if ftp.isFile('cac_runtests.m')
            ftp.delete('cac_runtests.m');
        end
        ftp.put(localPath,'cac_runtests.m');
        files = ftp.list('',1);
        foundruntests = 0;
        for i = 1:length(files)
            if strcmp(files{i}.name,'cac_runtests.m')
                foundruntests = 1;
                break;
            end
        end
    ...
    function runJobSubmit(sched)
        fprintf('Attempting to submit a simple test job to Red Cloud.\n');
        ...
        try
            ...
            if ismac() && ~isempty(strfind(exc.message,'Keystore was tampered with, or password was incorrect'))
            	...
            	fprintf('Once complete, rerun cac_runtests(sched,2)\n');
            else
            	disp(exc.message);
            end
        end
    ...
    function getFreeCores()
        %Get Red Cloud Usage, which may require a updateContrib, which may require a
        %path addition
        CACHome = fileparts(which('cac_runtests'));
        ...
        if ~exist('getRedCloudStatus.m','file')
            %is it because contrib still isn't on the path?
            if ~exist('updateContrib.m','file')
                fprintf('Odd, can''t seem to find updateContrib after path add, might not be able to give you Red Cloud current usage.\n');
        ...
        if ~exist('getRedCloudStatus.m','file')
            fprintf('Can''t seem to retrieve Red Cloud status.  Hopefully we''re not busy!\n');
        else
            getRedCloudStatus();
        end
    end
    ...
    for i=1:2
        if (sum(sum(result{i})) == 0)
            warning('cacscheduler:DistributedJobTest','Distributed Job test should have returned non-zeros matrices');
            fails = fails+1;
        end
    end
    end
    ...
    if ~strcmp(j.state,'finished')
        warning('cacscheduler:DistributedJobCancelTest','Distributed Cancel test should have returned "finished" state');
        fails = fails+1;
    end
    if ~strfind(j.tasks(1).ErrorMessage, 'canceled')
        warning('cacscheduler:DistributedJobCancelTest','Distributed Job cancel test should have returned reason for finished state');
        fails = fails+1;
    end
    end
    ...
    if ~strcmp(j.state,'finished')
        warning('cacscheduler:ParallelJobCancelTest','Parallel Job cancel test should have returned "finished" state');
        fails = fails+1;
    end
    if ~findstr('canceled',j.tasks(1).ErrorMessage)
        warning('cacscheduler:ParallelJobCancelTest','Parallel Job cancel test should have returned reason for finished state');
        fails = fails+1;
    end
    end
