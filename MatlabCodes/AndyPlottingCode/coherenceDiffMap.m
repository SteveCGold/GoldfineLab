...
%7.9.13 version 6 to allow for use with EGI datasets
...
%     dh.chanlocVariable=eegp_makechanlocs(char(dh.Diff.ChannelList),eegp_opts);
    if strcmp(dh.TGTandDiff.ChannelList(1),'E1') %if EGI129
        dh.chanlocVariable = pop_readlocs('GSN-HydroCel-129.sfp');%this file comes with EEGLAB and also is in EGI
        dh.chanlocVariable=dh.chanlocVariable(4:end);%first three are empty
    else
...
% dh.eegp_opts=eegp_defopts_eeglablocsAndy;%removed 7/9/13 since inaccurate
dh.hopts.Laplacian_nbrs_interior=5;%so more lines filled in
if strcmpi('E1',dh.TGTandDiff.ChannelList(1)) %assuming first channel is E1
    disp('assuming EGI 129 channel array');
    eegp_opts=eegp_defopts_eeglablocsAndy('EGI129');
    dh.Hjorth=cofl_xtp_makemont(dh.TGTandDiff.ChannelList,[],dh.hopts,eegp_opts);
else
    dh.Hjorth=cofl_xtp_makemont(dh.TGTandDiff.ChannelList,[],dh.hopts);
end
...
% and variables don't span functions
    ...
    function Done_callback(src,eventdata,h,dh,fh)
        ...
        if removeEdge
            if strcmpi('E1',dh.Cohfile.ChannelList(1)) %assuming first channel is E1
                eegp_opts=eegp_defopts_eeglablocsAndy('EGI129');
                dh.Hjorth=cofl_xtp_makemont(dh.TGTandDiff.ChannelList(~dh.edgeInd),[],dh.hopts,eegp_opts);
            else
                dh.Hjorth=cofl_xtp_makemont(dh.TGTandDiff.ChannelList(~dh.edgeInd),[],dh.hopts);%to redo a nicer frame with just relevant channels
            end
...
%%
    function plotCohMapDo(startFreq,range,spacing,plot3D,alpha,fh,dh)
        ...
        if strcmpi('E1',dh.TGTandDiff.ChannelList(1)) %assuming first channel is E1
            opts.eegp_opts=eegp_defopts_eeglablocsAndy('EGI129'); %added 7/9/13 so can use EGI dataset. 
        end
