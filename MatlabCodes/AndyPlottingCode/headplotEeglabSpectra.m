...
%4/1713 version 3 uses the chanlocs variable now created in runEeglabSpectra mainly for the bipolar
%channels 
%8/8/13 give option for use of 32 channel EGI. 8/13/13 fixed so that it
%plots the horizontal axis correctly since is coming up backwards for some
%reason.
%9/6/13 found that all EGI's are "backwards" but it's because the theta is
%negative sign. So will just use theta's sign to determine Y.
...
if strcmpi(dh.spectra(1).ChannelList{1},'E1')%if an EGI file
    if length(dh.spectra(1).ChannelList)==33
        dh.cv=pop_readlocs('GSN-HydroCel-32.sfp');
    else
...
%    opposite sign to the EGI129 and JV's code so need to flip them all
         for d3=1:length(dh.cv)
             dh.cv(d3).Y=-dh.cv(d3).Y;
         end
else
    if isfield(dh.spectra(1),'chanlocs') %since chanlocs only saved after 4/17/13
        dh.cv=dh.spectra(1).chanlocs;%new 4/17/13
    else
...
%     
        ...
        if sum(strcmpi({dh.cv.labels},'Cz'));%if there is a channel named Cz like in all laplacian spectra
            cc=dh.cv(strcmpi({dh.cv.labels},'Cz'));
        elseif sum(strcmpi({dh.cv.labels},'FZ-CZ'));%for bipolar
            cc=dh.cv(strcmpi({dh.cv.labels},'FZ-CZ'));%seems like a good choice though might be others
        else
