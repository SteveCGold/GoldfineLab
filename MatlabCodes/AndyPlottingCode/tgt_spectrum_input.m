function [dz,vdz,Adz]=tgt_spectrum_input(S1,S2,p,plt,f)
% based on two_group_test_spectrum but takes in the spectrum instead of the
% fourier transform. Requires that data come in as frequency x sample. 
%
%ORIGINAL INSTRUCTIONS WHICH AREN'T VALID HERE:
...
%
% Usage:
% [dz,vdz,Adz]=two_sample_test_spectrum(J1,J2,p,plt,f)
%
% Inputs:
...
if nargin < 2; error('Need two sets of Spectra'); end;
if nargin < 4 || isempty(plt); plt='n'; end;
% 
% Test for matching dimensionalities
%
m1=size(S1,2); % number of samples, condition 1
m2=size(S2,2); % number of samples, condition 2
dof1=m1; % degrees of freedom, condition 1
dof2=m2; % degrees of freedom, condition 2
if nargin < 5 || isempty(f); f=size(S1,1); end;
