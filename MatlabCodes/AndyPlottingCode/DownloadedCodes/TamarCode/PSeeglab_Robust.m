...
mtmethodflag = input('Use nonrobust (0) or robust (1) estimator? [1]:');
if isempty(mtmethodflag); mtmethodflag = 1; end;
if mtmethodflag
    mtmethod = struct('class', 'two-tier', 'scalefactor', struct('spectrum', [], 'error', []), 'tier', struct('estimator', {'mean', 'median'}));
else
    mtmethod = struct('class', 'standard');
end
...
for numToRun=1:length(setfilename)
    ...
    runEeglabSpectra_Robust(alleeg,resopts, mtmethod);
    clear alleeg
end