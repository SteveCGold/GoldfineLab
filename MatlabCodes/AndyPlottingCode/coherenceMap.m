...
%7/9/13 version 6 add option for EGI 129
...
if strcmpi('E1',dh.Cohfile.ChannelList(1)) %assuming first channel is E1
    ...
    dh.Hjorth=cofl_xtp_makemont(dh.Cohfile.ChannelList,[],dh.hopts,eegp_opts);%[] means same channels out as goes in. 
else
...
% uiwait
    ...
    function Done_callback(src,eventdata,h,dh,fh)
        ...
        if removeEdge
            if strcmpi('E1',dh.Cohfile.ChannelList(1)) %assuming first channel is E1
                eegp_opts=eegp_defopts_eeglablocsAndy('EGI129');
                dh.Hjorth=cofl_xtp_makemont(dh.Cohfile.ChannelList(~dh.edgeInd),[],dh.hopts,eegp_opts);
            else
