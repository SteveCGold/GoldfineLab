...
%                                   converted
%withPhoticv1 2/11/11 AG            add in cutting of third cell of
%                                   filedata.data which is photic
%DON'T FORGET TO UPDATE VERSION NUMBER BELOW!!!


function [snippets status]= xtp_cutSnippetsWithPhotic(filedata, epochlist, params)

funcname = 'xtp_cutSnippetsWithPhotic';
version = 'v1';
...
%     else 
        convertUnits = 1;
        fprintf('Converting units from %s to %s.\n', filedata.metadata.units, params.units);
%     end
end
...
for e=1:numepochs
    ...
    if isempty(snippet)
        ...
        %AG [] add in code to pull out matching photic data here:
        photic=filedata.data{3}(startindex:endindex,1); %AG strings of 'ON' or 'OFF'
        snippets.photic{snippetnum}=strcmpi(photic,'ON'); %AG should give 1 if on and 0 if 'OFF' need to check
