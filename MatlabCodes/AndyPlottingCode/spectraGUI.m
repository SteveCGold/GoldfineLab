...
%8/29/13 added code so if plot figure is closed it won't lead to an error
%8/16/13 version 3 once set will calls headplotLocations to determine locations and then
%plots with headplotSpectra
...
h.controlFig = figure(...
    ...
    'Position',[5 50 30 25],...
    'Visible','on',...
    'Tag','0');%to use to track number of plots for determining colors

% %    'Position',[103.833333333333 43.0833333333333 22.8333333333333 18.4166666666667]


h.sf=figure;%spectra figure
set(h.sf,'Visible','off');%so not visible at the beginning
...
%         if h.montage==1 %laplacian montage %his is an error found 6/15/12
        ...
        %%
        %make figure
        try get(h.sf,'Type') %if figure was closed by user 
        catch %if gives an error because it was closed
            h.sf=figure;%create new figure
        end
        
        figure(h.sf);
        
        ct=str2double(get(h.controlFig,'Tag'));%get current number of plots, starts at 0 from above code
        
        set(h.controlFig,'Tag',num2str(ct+1));%add 1 for new total number
...
%subcode to clear
    function clearSpecFig(src,event,h)
        clf(h.sf);
        set(h.controlFig,'Tag','0');%back to zero plots for coloring next one
    end
