%5/2/12 based on current version of eegplot but simply modifies REJECT
%button command to not get the data and to not close the window (removes
%all the commands it runs other than what I put in using 'command'). AMG
%
%
...
function [outvar1] = eegplotAndy(data, varargin); % p1,p2,p3,p4,p5,p6,p7,p8,p9)
...
if ~isstr(data) % If NOT a 'noui' call or a callback from uicontrols
  ...
  if isstr(g.eloc_file) | isstruct(g.eloc_file)  % Read in electrode names
      ...
      eegplotAndy('setelect', g.eloc_file, ax1);
  end;
...
u(22) = uicontrol('Parent',figh, ...
  ...
  acceptcommand =  tmpcom; % quitting expression
...
%  set(figh, 'busyaction', 'cancel');
  ...
  eegplotAndy('scaleeye', [], gcf);
  if strcmp(lower(g.scale), 'off')
	  eegplotAndy('scaleeye', 'off', gcf);
  end;
  
  eegplotAndy('drawp', 0);
  eegplotAndy('drawp', 0);
  if g.dispchans ~= g.chans
  	   eegplotAndy('zoom', gcf);
  end;  
  eegplotAndy('scaleeye', [], gcf);
...
%        keyboard;  
    ...
    set(ax1, 'ylim',[g.elecoffset*g.spacing (g.elecoffset+g.dispchans+1)*g.spacing] );
	 ...
	 if g.children ~= 0
		...
		eegplotAndy( 'drawp', p1, p2, g.children);
		figure(figh);
	 end;	  
...
{ 'Number of channels to display:' } , 'Change number of channels to display', 1,  { num2str(g.dispchans) });
   ...
   set(gcf, 'UserData', g);
   eegplotAndy('updateslider', fig);
   eegplotAndy('drawp',0);	
   eegplotAndy('scaleeye', [], fig);
   return;
  ...
  case 'loadelect' % load channels
	...
	eegplotAndy('setelect',[ inputpath inputname ],AXH0);
	return;
    ...
    if strcmp(get(fig, 'SelectionType'),'normal');
        ...
        if g.incallback ~= 1 % interception of nestest calls
            ...
            if (tmppos(1) >= 0) & (tmppos(1) <= highlim),
                ...
                set(fig,'UserData', g);
                eegplotAndy('drawp', 0);  % redraw background
            end;
        end;
    end;      
   otherwise
