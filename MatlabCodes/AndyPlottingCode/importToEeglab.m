%%
function importToEeglab

%Updated version - now has options for photic stims.
...
while 1
    for a=1:length(UniqueEvents) %display all events with a number
        fprintf('Event Type %d : %s \n', a, UniqueEvents{a});
    end
    ...
    [EEG_ep]=pop_epoch(EEG,UniqueEvents(pickEpoch),[it ft]);
