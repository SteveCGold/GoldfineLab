...
f.topoFreq=uicontrol('Parent',f.TGTtab_c1,'style','edit',...
    'backgroundcolor','w');
...
%             sgfigName=get(sgfig,'Name');
            title(['Topoplots at ' num2str(plotFreq) ' Hz']);
%             set(sgfig,'Name',[sgfigName ' ' num2str(plotFreq) ' Hz']);
            for nt=1:size(SgBaseCorr,1)%for each time point
                ...
                pvalPlotInd=pval(closestFreqInd,nt,:)<str2double(get(f.p_cutoff,'string'));%the value chosen by user  
                topoplot(10*log10(squeeze(SgBaseCorr(nt,closestFreqInd,:))),chanlocs(channels),...
