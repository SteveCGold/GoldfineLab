...
%frequency)
%v3 with topoplot (though started editing in version 2). Also now allows
%spectrogram with mean subtraction chosen (subtracts off full time range).
%Also allows TGT calculation with mean subtraction just using full time
%range.
%v4 1/15/14 fixing bugs: 1) tapers 1 was never set so is now set based on
%tapers 2
% Also add in option for topoplot +/- certain frequency so more confident
% in results.
...
elseif strcmpi(eeg(1).chanlocs(1).labels,'bA1') %if biosemi
    etable=biosemi_equivtable_Andy; %columns of biosemi, output
    output=etable(:,2);
    for ee=1:length(eeg(1).chanlocs)
        if sum(strcmpi(eeg(1).chanlocs(ee).labels,etable(:,1)))
                eeg(1).chanlocs(ee).labels=output...
...
f.subTypeBox=uibuttongroup('Parent',f.plottab_v1);%box to contain mutually exclusive radio buttons
    uicontrol('Parent',f.subTypeBox,'style','radiobutton','string',...
        'No subtraction','units','normalized','position',[0 0.75 1 0.1]);%first option so is the default
    f.meansub=uicontrol('Parent',f.subTypeBox,'style','radiobutton',...
        'string','Subt Mean','units','normalized','position',[0 0.6 1 0.1]);%for Subtract the mean (= full baseline)
    f.basesub=uicontrol('Parent',f.subTypeBox,'style','radiobutton','value',...
        0,'string','Sub Baseline','units','normalized','position',[0 0.45 1 0.1]);%for baseline subtract.
    f.matchedsub=uicontrol('Parent',f.subTypeBox,'style','radiobutton','value',...
        0,'string','Sub Second Half','units','normalized','position',[0 0.3 1 0.1]);%For subtracting the matched time from 2nd half
    f.othereegsub=uicontrol('Parent',f.subTypeBox,'style','radiobutton','value',...
        0,'string','Sub Second EEG','units','normalized','position',[0 0.15 1 0.1]);%For subtracting the matched time from 2nd half
    f.baseandothersub=uicontrol('Parent',f.subTypeBox,'style','radiobutton','value',...
        0,'string','Sub Base & 2nd EEG','units','normalized','position',[0 0 1 0.1]);%For subtracting the matched time from 2nd half
...
%until sg is run
set(f.plottab_v1,'Sizes',[-0.6 -1.8 -.5 -0.5 -1 -1]);
...
f.TGTtab_c1_h=uiextras.HBox('Parent',f.TGTtab_c1);%horizontal box on bottom
f.topoFreq=uicontrol('Parent',f.TGTtab_c1_h,'style','edit',...
    'backgroundcolor','w');
f.topoPlusMinus=uicontrol('Parent',f.TGTtab_c1_h,'style','edit',...
    'backgroundcolor','w');
set(f.TGTtab_c1,'Sizes',[-0.7 -0.1 -0.2]);
...
set(f.plotSpec_WithTGTFDR,'Callback',{@plotSpec_WithTGT,eeg(1).chanlocs,f,1,0});%First flag for use local FDR, second for topoplot
set(f.plotSpec_WithTGTChoose,'Callback',{@plotSpec_WithTGT,eeg(1).chanlocs,f,0,0});
set(f.plotTopo_WithTGTFDR,'Callback',{@plotSpec_WithTGT,eeg(1).chanlocs,f,1,1});%12/23/13
% disabled since not sure whether to do FDR by frequency or channel.
% Disabled below in TGT calculation code
...
%also calculates the baseline to show user potential values
    function calculateFRandBase(hObject,eventdata,f,eeg) %just gets eeg(1)
        fres=(str2double(get(f.tapers,'string'))+1)/str2double(get(f.mw_w,'string'));
        fresString=sprintf('Freq Res is %.2f Hz',fres);
        set(f.fr,'string',fresString);
        set(f.topoPlusMinus,'string',sprintf('%.2f',(ceil(fres*100)/100)/2));%divided by two so that +/- gives full freq resolution
        %Using ceil to make sure to be inclusive e.g. if numbers spaced by
        %2/3 then 0.33 won't capture them but this code will round up to
        %0.34
...
%% function to turn off plotting once new channels chosen and select channels
    function channels_Callback(hObject,eventdata,f)
        if get(f.ten20,'value')%if 10-20 chosen
            if strcmpi(f.ChannelList{2},'bA2') %if biosemi since missing F3 and others
                ten20list={'Cz','CPz','PO7','O1','Pz','POz','Oz','Iz','O2','PO8','P8',...
                    'TP8','C2','C4','C6','T8','FT8','F8','AF8','Fp2','Fpz','AFz','Fz',...
                    'FCz','Fp1','AF7','F7','FT7','C1','C3','C5','T7','TP7','P7'};
            else %standard
        ...
        set(f.plotTopo_WithTGTChoose,'enable','off');
        set(f.plotTopo_WithTGTFDR,'enable','off');
...
%in the figure using guihandles and then guidata
    function calculateSG(hObject,eventdata,f,eeg)
        ...
        params.tapers(1)=(params.tapers(2)+1)/2;%fixed 1/15/14
    ...
    function plotSG(hObject,eventdata,f)
        ...
        for c=1:size(Sg,4)%only plotting from channels chosen before
            ...
            if get(f.plotAvg,'value') %if plot avg, otherwise baseline subtractions not programmed though could be
                ...
                elseif get(f.basesub,'value') || get(f.baseandothersub,'value') %if want to do baseline subt
                    ...
                    %calculate baseline corrected for first spectrogram
                    ...
                    if get(f.baseandothersub,'value')%if comparing two baseline subtracted spectrograms
                        meanBaseline2=mean(spec(2).Sg(btr,:,:,c),1);%[]check this!
                        meanBaseline2=mean(meanBaseline2,3);%next mean across trials
                        meanBaseline2=repmat(meanBaseline2,size(spec(2).Sg,1),1);%make a 2D matrix
                        %Next take mean first to conver to 2D then divide by
                        %baseline (same as subtracting the logs)
                        SgBaseCorr2=squeeze(mean(spec(2).Sg(:,:,:,c),3))./meanBaseline2;%dividing by power is same as subtracting log
                        
                        imagesc(t_plot,freq(pfr),10*log10(SgBaseCorr(:,pfr)./SgBaseCorr2(:,pfr))',cLimMandB);
                        titleText=sprintf('(Baseline (%.2f to %.2f sec) and Second EEG subtracted Off First)',bStart,bEnd);
                    else
...
%% Function for calculating TGT
    function calculateTGT(hObject,eventdata,f)
        ...
        %if comparing to chosen or mean baseline 
        if ~get(f.matchedsub,'value') && ~get(f.othereegsub,'value') && ~get(f.baseandothersub,'value')
            %first make sure that baseline values are selected or for mean
            %use full baseline
            if get(f.meansub,'value')
                baseStart=spec(1).t(1);
                baseEnd=spec(1).t(end);
            else
...
%             end
        ...
        elseif get(f.baseandothersub,'value') %if doing subraction of baseline and then comparing two spectra
            disp('No p-value calculation yet for comparing baseline subtracted, setting all to 1');
            spec(1).tgtp_value=ones(size(spec(1).Sg,2),size(spec(1).Sg,1),size(spec(1).Sg,4));% Initialise the p_values and dz
            spec(1).dz=spec(1).tgtp_value;
            
            for c=1:size(spec(1).Sg,4)
                %get the baseline
                btr=spec(1).t>=str2double(get(f.BaselineStart,'string')) & ...
                    spec(2).t<=str2double(get(f.BaselineEnd,'string'));
                if ~sum(btr) %if no values between range
                    errordlg('No times within baseline range');
                end
                
                %calculate for first and second spectra
                meanBaseline=mean(spec(1).Sg(btr,:,:,c),1);%mean of all times
                meanBaseline=mean(meanBaseline,3);% mean across trials
                meanBaseline=repmat(meanBaseline,size(spec(1).Sg,1),1);%make a 2D matrix same size as rest of data
                meanBaseline2=mean(spec(2).Sg(btr,:,:,c),1);%[]check this!
                meanBaseline2=mean(meanBaseline2,3);%next mean across trials
                meanBaseline2=repmat(meanBaseline2,size(spec(2).Sg,1),1);%make a 2D matrix                
                ...
                %Next take mean first to conver to 2D then divide by
                %baseline (same as subtracting the logs)
                SgBaseCorr(:,:,c)=squeeze(mean(spec(1).Sg(:,:,:,c),3))./meanBaseline;
                SgBaseCorr2(:,:,c)=squeeze(mean(spec(2).Sg(:,:,:,c),3))./meanBaseline2;%dividing by power is same as subtracting log
                
                %calculate for second spectra if comparing baseline
                %subtracted spectra
                SgBaseCorr(:,:,c)=SgBaseCorr(:,:,c)./SgBaseCorr2(:,:,c);
            end
            
            [spec]=calculatesurrogatepvalues(spec,SgBaseCorr,btr);
...
%         set(f.plotTopo_WithTGTFDR,'enable','on');%12/23/13 - disabled
%         since not sure whether to make FDR by frequency or channel
...
%% Function for plotting the -log10 of the TGT p-values only
    function plotTGTp(hObject,eventdata,f)
        ...
        elseif get(f.baseandothersub,'value')
            set(gcf,'Name',['TGT p Base Corr.' f.filename{1} ' vs ' f.filename{2}]);
        end
        channels=str2double(get(f.channelsSelected,'string'));
...
%% Function for plotting the spectrogram or topoplot with non significant values hidden
    function plotSpec_WithTGT(~,~,chanlocs,f,useFDR,topo)
        %code based on the spectrogram plotting code above but suppress
        %non-significant TGT values. chanlocs needed for topoplot. topo is
        %to do a topoplot rather than a spectrogram
        ...
        elseif get(f.baseandothersub,'value')
            set(sgfig,'Name',['Sig. Sgram Base Corr.' f.filename{1} ' vs ' f.filename{2}]);
        end
        ...
        %plotting frequency range for calculating FDR and plotting
        pfr=(freq>=str2double(get(f.FRangeStart,'string'))) & (freq<=str2double(get(f.FRangeEnd,'string')));
        
        %plot the data based on the chosen options. Create the spectrograms
        %and either plot them or save for use in topoplot code below. 
        for c=1:size(Sg,4)%only plotting from channels chosen before
             
            %Here calculate the local FDR cutoff. Can choose if to
            %include baseline in the calculation
            
            %if want to use the baseline or baseline isn't used her
            if get(f.baseInFDR,'value') || get(f.matchedsub,'value') || get(f.othereegsub,'value') || get(f.baseandothersub,'value')
                pvaluesInPFR=pval(pfr,:,c);%for the PFR and channel
            else %if want to ignore the baseline in the local FDR calculation
                if get(f.meansub,'value') %if mean subtraction selected use full basleine
                    baseStart=t(1);
                    baseEnd=t(end);
                else
                    baseStart=str2double(get(f.BaselineStart,'string'));
            ...
            if useFDR %use alpha cutoff determined locally by FDR for each channel
                local_Pcut(c)=Anitafdr(pvaluesInPFR(:),0.05);%FDR just for each channel and PFR
                if isempty(local_Pcut)
                    local_Pcut(c)=NaN;
                end
            else %use the cutoff chosen by user
                local_Pcut(c)=str2double(get(f.p_cutoff,'string'));%the value chosen by user               
            end
            %
            
            if get(f.meansub,'value') || get(f.basesub,'value') || get(f.baseandothersub,'value')               
                if get(f.basesub,'value') || get(f.baseandothersub,'value')   %if baseline subtraction used
                    %btr is baseline time range
                ...
                if get(f.meansub,'value') %then use full time range for baseline
                    btr=true(1,length(t));%use full time range
                end
                
                %calculate for first spectra
                ...
                %Next take mean first to conver to 2D then divide by
                %baseline (same as subtracting the logs)
                SgBaseCorr(:,:,c)=squeeze(mean(Sg(:,:,:,c),3))./meanBaseline;
                
                %calculate for second spectra if comparing baseline
                %subtracted spectra
                if get(f.baseandothersub,'value') 
                    meanBaseline2=mean(spec(2).Sg(btr,:,:,c),1);%[]check this!
                    meanBaseline2=mean(meanBaseline2,3);%next mean across trials
                    meanBaseline2=repmat(meanBaseline2,size(spec(2).Sg,1),1);%make a 2D matrix
                    SgBaseCorr2=squeeze(mean(spec(2).Sg(:,:,:,c),3))./meanBaseline2;%dividing by power is same as subtracting log
                    SgBaseCorr(:,:,c)=SgBaseCorr(:,:,c)./SgBaseCorr2;
                end
...
%                     keeps getting cut in half each time through!
                    %Next plot 1st half minus second half
                    SgBaseCorr(:,:,c)=squeeze(mean(Sg(1:length(t_plot),:,:,c),3))./...
                        squeeze(mean(Sg(length(t_plot)+1:end,:,:,c),3));
            elseif get(f.othereegsub,'value') %if subtracting second EEG from first
                SgBaseCorr(:,:,c)=squeeze(mean(Sg(:,:,:,c),3))./...
                    squeeze(mean(spec(2).Sg(:,:,:,c),3));
            end
                
            %above created SgBaseCorr which is time x freq x channel
                      
            %set alpha
            if isnan(local_Pcut(c)) %if no significant values
                alpha=repmat(0.2,sum(pfr),size(pval,2));%frequency x time
            else
                alpha=(pval(pfr,:,c)>local_Pcut(c))*0.2;%find the non-significant values and set to be transparent
                alpha(alpha==0)=1;%set significant values to be 1
            end
            
            if ~topo %then create a spectrogram, otherwise leave this loop and make topoplot
                %plot 
                subplot(length(channels),1,c);
                imag=imagesc(t_plot,freq(pfr),10*log10(squeeze(SgBaseCorr(:,pfr,c)))',cLimMandB);
                ...
                axis xy;
                colorbar
                pcutAsString=sprintf('%0.2e',local_Pcut(c));
        ...
        if topo %make topoplot figure instead
            %() For now do just with one frequency but can put a GUI in the
            %topoplot figure to do different frequency or time base
            
            %plot at chosen frequency
            chosenFreq=str2double(get(f.topoFreq,'string'));
            if isempty(chosenFreq)
                errordlg('Choose Frequency for topoplot')
                return
            end
            
%             %if doesn't match, then pick closest
%             [~,closestFreqInd]=min(abs(freq-chosenFreq));%chose the one closest to chosen which is closest to 0
%             plotFreq=freq(closestFreqInd);

            %determine plotting frequency range based on chosen
            %frequency+/- chosen range which by default is the freq
            %resolution
            cf_range=str2double(get(f.topoPlusMinus,'string'));
            pfr=freq>=chosenFreq-cf_range & freq<=chosenFreq+cf_range;
            
            %determine number of topoplots based on number of time points
            [nr,nc]=subplotNumCalc(size(SgBaseCorr,1));
%             sgfigName=get(sgfig,'Name');
            title(['Topoplots at ' num2str(chosenFreq) ' Hz']);
%             set(sgfig,'Name',[sgfigName ' ' num2str(plotFreq) ' Hz']);
            for nt=1:size(SgBaseCorr,1)%for each time point
                subplot(nr,nc,nt);
                %pval is freq x timepoint x channel
                pvalPlotInd=squeeze(mean(pval(pfr,nt,:),1)<str2double(get(f.p_cutoff,'string')));%the value chosen by user
                topoplot(10*log10(squeeze(mean(SgBaseCorr(nt,pfr,:),2))),chanlocs(channels),...
                    'maplimits',cLimMandB,'contourvals',+pvalPlotInd,'numcontour',1);
                text(-.1,-.7,sprintf('%.2f',t_plot(nt)));
            end
