...
function [S,f,savename]=runEeglabSpectra(alleeg,opts,combineddata)
...
%laplacian
%3/6/12 version 6 gives option to use channel list in the opts.subset
%field which is just a vector of channel numbers. Note this occurs after creating
%Laplacian and bipolar so the channels are created with all original
%leads and numChannels is changed to the number in the subset.
%4/17/13 save the chanlocs so have the location of the bipolar channels for
%use in headplotEeglab codes. Also fixed bipolar naming since was cutting
%off too many characters
...
if opts.bip
    [data,ChannelList,chanlocs]=MakeBipolar(data,chanlocs);%added chanlocs as output here 4/17/13
    savename=[savename(1:end-4) '_Bip_PS'];
...
%%
%choose specific channels to run on , note this occurs after creating
%Laplacian and bipolar.
numChannelsOrig=numChannels; %so can use later
if opts.subset
    data=data(opts.subset,:,:);
    chanlocs=chanlocs(opts.subset);
    numChannels=size(data,1);
end
%%
%Run Power spectra
...
for ps=1:size(data,1) %for each channel
    [S(:,ps) f Serr(:,:,ps)]=mtspectrumc(squeeze(data(ps,:,:)),params); %stack each spectra in a column which is way that ULT gives the output
...
%     if isfield(alleeg,'bipolar') && alleeg.bipolar %since ChannelList comes from makeBipolar
    ...
    save (savename,'S','f','Serr','frequencyRecorded','numChannels','numChannelsOrig','icadata','ChannelList','dataCutByChannel','params','chanlocs');
    fprintf('%s created\n',savename);
