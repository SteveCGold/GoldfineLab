...
function subplotSpectra(spectra1, spectra2, spectra3, subset, figuretitle, spectra1label, spectra2label, spectra3label, numrows, numcols, graphcomments, xstart, xend)
...
%pectra2label','spectra3label',numrows,numcols,'graphcomments', xstart, xend)
...
%version 2.3 12/15 added xstart and xend to have start and end points to
             %xaxis. Also increased font size of y, x axes and legend (but
             %not title)


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Defaults - feel free to modify but ensure the Channel List matches the order that
%appears from graph outputs in the browser.
%FrequencyRecorded=256 %256 for EMU40 and 200 for all previous headboxes. Essential for x axis and secondsofdataused
%EKDBChannelsReverseOrder={'Cz-Pz' 'Fz-Cz' 'T6-O2' 'CP6-T6' 'T4-CP6' 'FC6-T4' 'F8-FC6' 'AF8-F8' 'Fp2-AF8' 'T5-O1' 'CP5-T5' 'T3-CP5' 'FC5-T3' 'F7-FC5' 'AF7-F7' 'Fp1-AF7' 'P4-O2' 'CP2-P4' 'C4-CP2' 'FC2-C4' 'F4-FC2' 'Fp2-F4' 'P3-O1' 'CP1-P3' 'C3-CP1' 'FC1-C3' 'F3-FC1' 'Fp1-F3'};
...
%if spectra1.ch exists (from batCrxULT) then set it to be the Channellist. If it is one of the 4 with the long names, assigned here. If used browser, need to type
%it in below where is says Channellist=... and put in one from the list.
...
ChannelList=EKDBTransverse; %here you define which channel list from above is used if need to define manually.
fprintf('data from browser used, ensure ChannelList variable set correctly in subplotSpectra\n');
end;
...
secondsofdatausedspectra1=sum((floor((spectra1.sMarkers(:,2)-spectra1.sMarkers(:,1)+1)/spectra1.movingwin(1,1))).*spectra1.movingwin(1,1)./FrequencyRecorded);

if isstruct(spectra2)
    secondsofdatausedspectra2=sum((floor((spectra2.sMarkers(:,2)-spectra2.sMarkers(:,1)+1)/spectra2.movingwin(1,1))).*spectra2.movingwin(1,1)./FrequencyRecorded);
else
...
if isstruct(spectra3)
    secondsofdatausedspectra3=sum((floor((spectra3.sMarkers(:,2)-spectra3.sMarkers(:,1)+1)/spectra3.movingwin(1,1))).*spectra3.movingwin(1,1)./FrequencyRecorded);
else
...
%this will open up the figure and assign it a size.
    ...
    %this is for the annotation of params with a version for 3 and a version for 2 spectra:
    ...
    annotation(figure(gcf),'textbox','String',{'params used:' 'tapers=' num2str(spectra1.params.tapers),...
        ...
        'Spectrum2 (sec used):' num2str(secondsofdatausedspectra2) 'Spectrum3 (sec used):' num2str(secondsofdatausedspectra3)},'FitBoxToText','off',...
    'Position',[0.01635 0.5581 0.07671 0.3851],'FontUnits','normalized'); %this uses normalized units.
        ...
        'pad=' num2str(spectra1.params.pad) 'fpass=' num2str(spectra1.params.fpass) 'err=' num2str(spectra1.params.err),...
        'MW=' num2str(spectra1.movingwin) 'Spectrum1 (sec used):' num2str(secondsofdatausedspectra1),...
        'Spectrum2 (sec used):' num2str(secondsofdatausedspectra2)},'FitBoxToText','off',...
...
%size rather than wraps if the textbox size changes.
    end;
...
if ischar(graphcomments)
 annotation(figure(gcf),'textbox','String',{graphcomments},'FitBoxToText','off',...
    ...
    for j=((numrows*numcols*(i-1))+1):min(length(subset),numrows*numcols*i); %designed to stop when subset reaches end but has error if don't use all graphs. so need to change not to max of subset but count of subset.
        ...
        set(gca,'xtick',[0:5:xend],'xlim',[xstart xend],'FontSize',24); %this sets the xticks every 5 and to begin and end at xstart and xend
        %xlabel('Frequency','FontSize',15); %this works but doesn't appear
                                            %when graphs grow
        ...
        plot(FrequencyRecorded*spectra1.f{1},(spectra1.S{1}(:,subset(j))),'b'); %f is normalized by browser so need to multiply by frequency recorded
        hold on;
        plot(FrequencyRecorded*spectra1.f{1},(spectra1.Serr{1}(:,:,subset(j))),'b','HandleVisibility','off');
         %by turning HandleVisibility off the legend won't label it since don't want to label the error
        title(ChannelList{subset(j)},'FontSize',14);
        legend(spectra1label);
        if isstruct(spectra2)%this line allows it to determine if a second spectra is entered
            plot(FrequencyRecorded*spectra2.f{1},(spectra2.S{1}(:,subset(j))),'r');
            plot(FrequencyRecorded*spectra2.f{1},(spectra2.Serr{1}(:,:,subset(j))),'r','HandleVisibility','off');
            legend(spectra1label,spectra2label);
        %would be nice to have a default name of the variable for legend if
        %not typed in
        end;
        if isstruct(spectra3)
            plot(FrequencyRecorded*spectra3.f{1},(spectra3.S{1}(:,subset(j))),'g');
            plot(FrequencyRecorded*spectra3.f{1},(spectra3.Serr{1}(:,:,subset(j))),'g','HandleVisibility','off');
