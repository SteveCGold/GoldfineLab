...
%                   recommend [100] hz.  If undefined, does not lowpass filter
%                   the data.
%
% Outputs:
...
% Copyright (C) Wearable Sensing, 2014.
%
...
if strcmp(g.importevent, 'on')
    ...
    for eventsearch = 1:length(Data(:,find(ismember(Montage,'Trigger'))))
        if Data(eventsearch,find(ismember(Montage,'Trigger'))) ~= 0
            if Data(eventsearch-1,find(ismember(Montage,'Trigger'))) == 0
                events = [events, eventsearch];
            end
        end
    end
    if ~isempty(events)
        for event = 1:length(events)
            EEG.event(event).type = ['trigger ', num2str(Data(events(event),find(ismember(Montage,'Trigger')))) ];
