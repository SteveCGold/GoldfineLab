...
%
% 6-24-10:  fixfac added to fix normalization error for tapers in files
% written by cofl_xtp_demo prior to this date; applied to data and
% surrogates
%
% 11-7-10:  revised to use the global frequency range defaults that work best for the longitudinal data sets
%
% TO DO:
...
if ~exist('headmap_leadsize') headmap_leadsize=30; end % size of markers for data points on wireframe
if ~exist('headmap_bandplots_chlabels') headmap_bandplots_chlabels=strvcat('C3','C4','CZ','O1','O2'); end % labels for bandplots
if ~exist('tapers_fix') tapers_fix=1; end %default is to fix taper normalization if tapers_fixed is not present or 0
%
...
%
plot_anal_choices{3}.desc='power and coherences on a headmap, by band';
...
auto=filldefault(auto,'tapers_fix',1);
%
...
if ~exist('tapers_fix') | (ifauto==1) tapers_fix=1; end
%
...
if (plot_anal_choice==1) & (ifauto==0)
    if_sppoints=getinp('1 for points representing surrogate data on spectral plots','d',[0 1],if_sppoints);
...
for ipca_ptr=1:length(ipca_list)
    for ipair_ptr=1:length(pair_list)
        for isurr_ptr=1:length(surr_list)
            ...
            pca_lab='pca: n/a';
            if ~isempty(r.cohgram.pca{1,ipca})
                pca_lab=r.cohgram.pca{1,ipca}.pca_label;
            end
            disp(sprintf('doing %s for %10s %s %s',...
                plot_anal_choices{plot_anal_choice}.desc_short,r.cohgram.labels{ipair},dstring,pca_lab));
            if ismember(plot_anal_choice,[1 3]) %spectrogram/coherogram plots or coherence plots
                ...
                %this section fixes the taper normalization 
                %if tapers are properly noramlized (multipied by sqrt(Fs), then nothing is done
                %if this was in error omitted (and tapers_fixed is absent), then we multiply by Fs
                %since spectral quantities involve squares of tapers
                fixfac=1;
                if ~isfield(r.cohgram,'tapers_fixed')
                    if (tapers_fix)
                        fixfac=r.metadata_recorded(1).srate;
                        S1concat=S1concat*fixfac;
                        S2concat=S2concat*fixfac;
                        S12concat=S12concat*fixfac;
                        disp(sprintf('tapers_fix=1; tapers_fixed not found; spectra fixed by multiplying by sampling rate, %6.2f',...
                            fixfac));
                    else
                        disp(sprintf('WARNING: tapers_fix=0; tapers_fixed not found; spectra NOT fixed by multiplying by sampling rate, %6.2f',...
                            fixfac));
                    end
                else
                    disp('tapers_fixed found; spectra already properly normalized.');
                end
            ...
            switch plot_anal_choice
                case 1 %simple plots of spectrograms, coherograms, pc weights
                    ...
                    for ic=1:nchans*(nchans+1)/2;
                        ...
                        if (if_sppoints==1) & isfield(r,'surrogate_info') %surrogate info used to find independent points
                            vplot_pts=fixfac*abs(getfield(r.surrogate_info{ipair},Slabels{ic})); %fixfac added 6-24-10
                        ...
                        for istat=1:length(stats_used.names)
                            ...
                            for irow=1:2+double(nsurrs>0)
                                ...
                                for kpc_dim=1:pca_headmap
                                    ...
                                    %collect data for headmap
                                    wireframe_data=[];
                                    for kpair_ptr=1:length(pair_list)
                                        kpair=pair_list(kpair_ptr);
                                        wireframe_data.pairlabel{kpair_ptr,1}=r.montage_analyzed{r.cohgram.pairs(kpair,1)};
                                        wireframe_data.pairlabel{kpair_ptr,2}=r.montage_analyzed{r.cohgram.pairs(kpair,2)};
                                        ...
                                        wireframe_data.paircolor(kpair_ptr,:)=cscale(cvalue,:);
                                        %
                                        nsig=1;
                                        if (nsurrs>0)
                                        ...
                                        wireframe_data.pairwidth(kpair_ptr)=headmap_linewidths(nsig);
                                        %row 3: significance row:  optionally use discrete color scale for significance
                                        if (irow==3 & headmap_sigstyle>0)
                                            wireframe_data.paircolor(kpair_ptr,:)=headmap_sigcolors(nsig,:);
                                        end
                                        if (irow==3 & headmap_sigstyle>1)
                                            wireframe_data.pairwidth(kpair_ptr)=headmap_linewidths_sig(nsig);
                                        end
                                    end
                                    %
                                    eegp_wireframe_data(wireframe_data,wireframe_opts);
                                    %
                ...
                case 3 % plots of coherence, etc in specific bands
                    ...
                    %
                    for kchan=1:2
                        headmap_labels_ptr=strmatch(deblank(r.montage_analyzed{r.cohgram.pairs(pair_list(ipair_ptr),kchan)}),headmap_labels,'exact');
                        if Sall_count(headmap_labels_ptr)==0 %first one
                            Sall(:,:,headmap_labels_ptr)=Sallpair{kchan}(:,:,ipair_ptr);
                            disp(sprintf('  spectrum collected for %4s from chan %1.0f of %8s',...
                                headmap_labels(headmap_labels_ptr,:),kchan,r.cohgram.labels{pair_list(ipair_ptr)}));
                        else
                            disp(sprintf('  spectrum  verified for %4s from chan %1.0f of %8s',...
                                headmap_labels(headmap_labels_ptr,:),kchan,r.cohgram.labels{pair_list(ipair_ptr)}));
                    ...
                    if ipair_ptr==length(pair_list)
                        ...
                        for band_type=1:nbandtypes
                            ...
                            % if single-channel (mergetype==2) also bandplots using eeglab's topoplot
                            % if single-channel (mergetype==2) also linegraphs positioned on the head
                            % colormap headplots of specdata_byband (mergetype=1 or 2)
                            %
                            %line graphs
                            figure;
                            ...
                            if (band_stat{band_type}.mergetype==2)
                                ...
                                chanlocs=eegp_makechanlocs(headmap_labels_use,eegp_opts); %for eeglab
                                ...
                                set(gcf,'Name',tstring);
                                set(gcf,'NumberTitle','off');
                                head_diam=2*max(eegp_cart('CZ')); %head diameter
                                ...
                                for ispec=0:nspecs
                                     if (ispec>0)
                                         ...
                                         headloc=[-head_diam*0.4 -head_diam*0.1 0]; %this is for a scale in the lower left
                                     end
                                     vals_axes=[0 0 max(cohgram_freqs);lims(2) lims(1) lims(1)]-repmat(hoff,1,3);
                                     xyzvals_axes=hxform*vals_axes+repmat(headloc',1,3);
                                     plot3(xyzvals_axes(1,:),xyzvals_axes(2,:),xyzvals_axes(3,:),'k'); hold on;
                                     if (ispec>0)
                                         tposit=xyzvals_axes*[1 -0.2 +0.2]';
                                         text(tposit(1),tposit(2),tposit(3),deblank(headmap_labels(ispec,:)));
                                     else
                                         tposit=xyzvals_axes*[1 0.4 -0.4]';
                                         text(tposit(1),tposit(2),tposit(3),sprintf('%5.0f',lims(2)),'FontSize',font_size);
                                         tposit=xyzvals_axes*[0 1.4 -0.4]';
                                         text(tposit(1),tposit(2),tposit(3),sprintf('%5.0f',lims(1)),'FontSize',font_size);
                                         tposit=xyzvals_axes*[-0.2 1.4 -0.2]';
                                         text(tposit(1),tposit(2),tposit(3),sprintf('%5.0f',0),'FontSize',font_size);
                                         tposit=xyzvals_axes*[-0.2 0.4 0.8]';
                                         text(tposit(1),tposit(2),tposit(3),sprintf('%5.0f',max(cohgram_freqs)),'FontSize',font_size);
                                     end
                                end
                                title(tstring,'FontSize',font_size);
                                axis vis3d;view(3);
                            ...
                            for iband=1:nbands;
                                ...
                                if (headmap_wires_ondata) %draw headmap wireframe on data
                                    eegp_wireframe(lap_coefs,r.montage_recorded,...
                                        setfield(wireframe_opts,'headmap_labels',headmap_bandplots_chlabels)); %plot headmaps without labels so that they don't interfere with points
                                end %headmap_wires_ondata
                                wireframe_data=[];
                                if (band_stat{band_type}.mergetype==1)
                                    %accumulate data for pairwise headmap plot
                                    for kpair_ptr=1:length(pair_list)
                                        kpair=pair_list(kpair_ptr);
                                        wireframe_data.pairlabel{kpair_ptr,1}=r.montage_analyzed{r.cohgram.pairs(kpair,1)};
                                        wireframe_data.pairlabel{kpair_ptr,2}=r.montage_analyzed{r.cohgram.pairs(kpair,2)};
                                        %find color value corresponding to data
                                        cvalue=round(size(cscale,1)*(specdata_byband(iband,kpair_ptr)-lims(1))/(lims(2)-lims(1)));
                                        cvalue=min(max(1,cvalue),size(cscale,1));
                                        wireframe_data.paircolor(kpair_ptr,:)=cscale(cvalue,:);
                                        wireframe_data.pairwidth(kpair_ptr)=headmap_linewidths(1);
                                    end
                                end
                                if (band_stat{band_type}.mergetype==2)
                                    %accumulate data for lead-oriented headmap plot
                                    for ispec=1:nspecs
                                        wireframe_data.leadlabel{ispec}=deblank(headmap_labels(ispec,:));
                                        %find color value corresponding to data
                                        cvalue=round(size(cscale,1)*(specdata_byband(iband,ispec)-lims(1))/(lims(2)-lims(1)));
                                        cvalue=min(max(1,cvalue),size(cscale,1));
                                        wireframe_data.leadcolor(ispec,:)=cscale(cvalue,:);
                                        wireframe_data.leadsize(ispec,:)=headmap_leadsize;
                                    end
                                end
                                eegp_wireframe_data(wireframe_data,wireframe_opts);
                                ht=title(sprintf(' >=%5.2fHz, <%5.2fHz',freqrange));
