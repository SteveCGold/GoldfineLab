...
function	[dip,xl,xu, ifault, gcm, lcm, mn, mj]=hartigan(xsamps)

% function	[dip,xl,xu, ifault, gcm, lcm, mn, mj]=hartigan(xsamps)
%
% Frozen from revised version of F. Mechler's HartigansDipTest (revised Sep 2 2002)
...
% This check the original Hartigan algorithm omitted, which leads to an infinite cycle if unchecked
%
% HartigansDipTest, like DIPTST, does the dip calculation for an ordered vector xsamps using
...
if all(find(xsign>0)<min(find(xsign<0)))
