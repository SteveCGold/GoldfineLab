function [Sc,Cmat,Ctot,Cvec,Cent,f]=CrossSpecMatNewc(data,win,params)
...
Ctot=zeros(length(findx),C); Cent=zeros(length(findx),1);
Cvec=zeros(length(findx),C,C);
for i=1:length(findx),
    [u s]=svd(squeeze(Sc(i,:,:)));s=diag(s);
    Ctot(i,:)=s./sum(s); Cent(i)=exp(mean(log(s)))/mean(s);             
    Cvec(i,:,:)=transpose(u);
end
